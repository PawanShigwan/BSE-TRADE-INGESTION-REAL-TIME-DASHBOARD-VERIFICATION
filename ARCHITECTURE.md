...
The design is structured this way because it directly maps each problem constraint → technical solution.

1. Mock BSE API → simulate the real problem

The left side represents the slow BSE source:
...
This lets you reproduce the actual assessment scenario without depending on a real BSE system.

2. Ingestion Engine → solves the 30-second problem
...
Worker → chunk 1 → BSE → response
Worker → chunk 2 → BSE → response
Worker → chunk 3 → BSE → response
...

Every HTTP request stays safely below 30 seconds.

That's why the diagram puts the Ingestion Engine between BSE and MongoDB. The dashboard shouldn't be responsible for pulling the exchange data.

3. MongoDB → persistent state

MongoDB has two important responsibilities:

trades
   ↓
Previously ingested trades

pullJobs
   ↓
Where the ingestion process currently is

This is important because the dashboard needs to show already-pulled trades immediately.
...
restart
       ↓
resume from cursor 1500

instead of starting from zero.

4. WebSocket → solves the real-time requirement

The dashboard cannot use:

setInterval(fetchTrades, 5000)

because the assessment explicitly prohibits polling.
...
The browser reacts immediately.

That's why WebSocket is shown as a separate real-time channel.

5. Dashboard → presentation layer

The dashboard is deliberately at the bottom because it consumes the data rather than controlling the ingestion process.
...
So even if BSE is currently taking several minutes to complete a pull, the dashboard remains usable.
